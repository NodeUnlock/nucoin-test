...
#if defined(NDEBUG)
# error "Nucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x400774ffff4679ab2fdecabb4d2a436d51a1a117698f9a12e273b9cd5b62a4b9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nucoin: starting difficulty is 1 / 2^12
...
{
    // Nucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 21024000); // Nucoin: 21m blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Nucoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Nucoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb7ea9e719e4305ce09276cf84ac2ef3fef2a081055d7c970340ebd1f774f1f45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Britain set for yet another scorcher today after hottest April day in 70 years. 20-04-18";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0404fb71d84551681969f6cd56341234ae408f0c845333a55f2822d79a44473d9a918b9a5439caba09cf0d05f8ada141085126d7ce39e11b16e6f079b155196a2d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524243250;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085647428;

        if (fTestNet)
        {
            block.nTime    = 1524243201;
            block.nNonce   = 387358838;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfb0a9bfde95bb1234aa3066e87b8489095c654cdb194698f08faa31da73e19d3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc4, 0xb7, 0xd1 }; // Nucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NucoinMiner(CWallet *pwallet)
{
    printf("NucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nucoin-miner");
    ...
    try { loop {
        ...
        printf("Running NucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NucoinMiner, pwallet));
}
